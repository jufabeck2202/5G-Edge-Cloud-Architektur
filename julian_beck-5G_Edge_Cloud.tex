...
Systembetrieb verfolgt. Edge Computing gilt dabei als eine Evolution von Cloud Computing, in dem es das Hosten von Anwendung an den
...
\\
\\
Die Abkürzungen \acrshort{mec} steht für  \acrlong{mec}. Dabei handelt es sich um eine Computer Architektur,
...
\\
\\
\acrshort{mec} ist nicht nur bei 5G zu finden, 
aber eine der wichtigesten Voraussetzung für diese. 
Die 5G-Architektur unterscheidet sich dadurch von den früheren Mobilfunkgenerationen, 
dass das MEC den latenzarmen Echtzeitzugang hoher Bandbreite zu den RAN-Daten umfasst. 
...
Neben der vorteilhaften Latenz und Bandbreite der MEC-Architektur ermöglicht die verteilte Rechenleistung eine höhere Anzahl von angeschlossenen Geräten, 
die untrennbarer Bestandteil der 5G-Bereitstellung und des sich entwickelnden Internet der Dinge (IoT) ist.   
\\
\\
...
Bei Cloud-Computing werden Rechenressourcen über das Netzwerk zur Verfügung gestellt.
...
\begin{itemize}
  ...
  \item \textbf{Security:} Wenn Daten in einer zentralen Cloud verarbeitet werden, kann dies zu Nachteilen über die Sicherheit eines Systems führen.
  ...
  das Kategorisieren der Daten. In dem ein Großteil der Verarbeitung am Rand des Netzwerks stattfindet wird Bandbreite eingespart.
  Dies optimiert den Datenfluss von lokalen Anwendungen und minimiert so die Betriebskosten einer zentralen Cloud.
  \item \textbf{Zuverlässigkeit:} Wenn eien Edge-Cloud Daten lokal speichern und verarbeiten können, verbessert dies die Zuverlässigkeit.
  Ein Unternehmen ist nicht auf die Verbindung zu einer zentralen Cloud angewiesen und eine vorübergehende Unterbrechungen der 
...
Das 5G Netzwerk besteht aus sogenannten Netzwerkfunktionen (Abschnitt: \ref{subsec:Network Function Virtualization}), die softwarebasiert sind und so nach Bedarf angepasst/skaliert werden können.
...
\begin{itemize}
  ...
  \item \textbf{\acrfull{nssf}} kann vom \acrshort{amf} verwendet werden, um bei der Auswahl der Network Slice-Instanzen zu helfen, die Anfrage von einem Gerät benötigt werden.
...
Die Kombination von \acrshort{sdn}, \acrshort{nfv}, \acrshort{nfvi} erlauben zusammen das Erstellen von \textit{Network Slices}. \cite{BARAKABITZE2020106984} 
\textit{Network Slices} ist eine der wichtigsten Technologien im 5G Netzwerk und
 ...
 Network-Slicing bietet Anwendungen wie IoT, wo es eine extrem große Anzahl von Nutzern geben kann, aber die insgesamt benötigte Bandbreite klein ist, deutliche Vorteile.
...
Container implementieren die Isolierung auf Betriebssystem Ebene und vermeiden so die Virtualisierung von Hardware und Treibern. 
...
Gegensatz zu \acrshort{vm}s. Typischerweise führt ein Container genau ein Service aus, was eine schnelle Migration ermöglicht.
\\
\\
...
als eine portable Laufzeit Umgebung für MEC-Dienste, die auch von MEC-Host zu einem weiteren migiriert werden können.
...
Folgendes Kapitel zeigt die von \acrfull{etsi} beschriebene
...
Referenz Architektur. Die Grafik \ref{fig:MecFramework} zeigt das MEC-Framework. 
Das MEC-Framework zeigt eine abstrahierte sicht der Komponenten eines MEC-Systems. Die Reference Architecture (Abbildung \ref{fig:mecarchitecture}) bietet 
